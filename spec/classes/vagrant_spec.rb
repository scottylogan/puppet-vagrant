...
describe 'vagrant' do
  ...
  describe 'when not specifiying a version' do
    it { should contain_package('Vagrant_1.7.3').with({
      :ensure   => 'installed',
      :provider => 'pkgdmg'
    })}
  end
  ...
  describe 'when installing bash completion' do
    ...
    it { should contain_package('Vagrant_1.7.3')}
    it { should contain_package('Vagrant_1.7.3').with_source('https://dl.bintray.com/mitchellh/vagrant/vagrant_1.7.3.dmg')}
